...
// BLP 2014-04-14 -- 
  $sql = "select siteId, allowAds, allowVideo, playbingo, playLotto, playtrivia, perRecent from appinfo";
  $rows = $S->queryfetch($sql);
  $n = count($rows);

  $sql = "select open, close from sites";
  $S->query($sql);
  $i = 0;
  while(list($open, $close) = $S->fetchrow('num')) {
    $rows[$i]['open'] = $open;
    $rows[$i++]['close'] = $close;
  }
} else {
  ...
  list($cnt) = $S->fetchrow('num');
// BLP 2014-04-14 --   
  $sql = "select siteId, allowAds, allowVideo, playbingo, playLotto, playtrivia, perRecent ".
         "from appinfo where siteId='$siteId'";
  $rows = $S->queryfetch($sql);
  $sql = "select open, close from sites where siteId='$siteId'";
  $S->query($sql);
  list($open, $close) = $S->fetchrow('num');
  $rows[0]['open'] = $open;
  $rows[0]['close'] = $close;
  $n = 1;
}
...
PlayTrivia: <b>{$v['playtrivia']}</b><br>
