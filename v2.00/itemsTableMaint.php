...
$sql = "select itemId, location from items";
$S->query($sql);
...
while(list($id, $loc) = $S->fetchrow("num")) {
  if(!file_exists("../" .$loc)) {
    ...
    $list .= "<li id='$id'>itemId=$id: <input type='checkbox' name='ids[]' checked value='$id'>$loc</li>\n";
  }
}
...
foreach($files as $file) {
  $sql = "select itemId from items where location='$file'";
  if(!$S->query($sql)) {
