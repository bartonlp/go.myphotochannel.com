...
(function(x) {
  x.COPYRIGHT = "&copy; 2017 myphotochannel";
...
// End trivia
  ...
  x.featureCnt = 0;
  x.itemList = new Array('announce', 'brand', 'product', 'info', 'video', 'bingo', 'lotto', 'trivia'); // CB categories
...
// BLP 2017-07-08 -- add trivia here for testing

function getTrivia() {
  // Get the info from the opentdb.com website via their api.
  // amount is the number of trivia elements.
  // category is the type of trivia questions, like 'sports' or
  // 'animals'
  // difficulty is how hard the questions is
  // type is either 'multiple' or true/false. We want multiple choice
  // questions.
  
  $.get('https://opentdb.com/api.php?amount='+g.sites.trivianum+'&category='+g.sites.triviacat+
        '&difficulty=easy&type=multiple',
        function(data) {
          // data.results has category, correct_answer, difficulty,
          // incorrect_answers[], question, and type.
...
function slideshow() {
  ...
  for(var n=0; image == null; ++n) {
    ...
    switch(cat) {
      ...
      case 'lotto':
        ...
        break;
      case 'trivia':
        num = g.inx.trivia++ % g.images.trivia.length;
        image = {};
        image.src = ''; // need this for 'replace' below
        image.mpcDur = g.sites.triviaqtime *1000;
        image.mpcTrans = 1000;
        image.mpcEffect = 'fade';
        image.mpcDesc = 'trivia';
        image.mpcType = 'trivia';
        image.mpcCat = 'trivia';
        image.mpcId = 'trivia';
        break;
      default:
  ...
  switch(g.image.mpcType) {
    case 'trivia':
      ...
      $("#show").append("<div>"+ // This div is required before the inner content!
                        "<style>#trivia {font-family: "+g.sites.triviafontstyle+
                        ";font-size: "+g.sites.triviafontsize+"rem;"+
                        "margin-top: 2%;text-align: center;}"+
                        "#trivia-title {margin-bottom: 0;}</style>"+
                        "<div id='trivia' class='htmlitem'>"+
                        "<h1 id='trivia-title'>Trivia</h1>"+
                        g.images.trivia[num]+"</div>"+
                        "</div>");
      break;
    case 'image':
...
function getBingo() {
  ...
  $.ajax({
    ...
    success: function(data) {
           ...
           g.bingoFreq = g.sites.bingoFreq; //data[1];
           g.bingoDrawNumber = g.sites.drawnumber; // = data[2];
...
function getInfo(callback) {
  $.ajax({
    ...
    success: function(data) {
           ...
           g.playtrivia = data.appInfo.playtrivia;
           if(g.playtrivia == 'yes') {
             getTrivia();
           }
...
jQuery(document).ready(function($) {
  ...
  $("#show").append("<img src='" + g.CONTENTPREFIX +
