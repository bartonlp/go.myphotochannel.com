...
if($_GET['page'] == 'startup') {
  require('../slideshow-v1.07/websocket/vendor/pusher/pusher-php-server/lib/Pusher.php');
...
if($_POST['page'] == 'approvephotos') {
  ...
  $sql = "select itemId, location from items where status='new' and siteId='$siteId'";
  if($S->query($sql) == 0) {
  ...
  while(list($id, $loc) = $S->fetchrow('num')) {
...
if($_POST['page'] == 'getItem') {
  ...
  $S->siteId = $_POST['siteId'];
  $sql = "select itemId, category, creationTime, creatorName, description, location, " .
...
function getItemRow($S, $q) {
  ...
  while(list($id, $cat, $time, $creator, $desc, $loc, $status, $type, $dur) =
...
<label for="$id-time-type">Time: </label>
<span id="$id-time-type">$time<br>$typeOfTime</span>
<hr>
