...
(function(x) {
  ...
  x.siteCode = siteCode;
  x.siteId = siteId;
  x.ajaxfile = "demo-slideshow.ajax.php"; // The ajax program we are using
...
g.pusherChannel = pusher.subscribe('demo-slideshow');;
...
function slideshow() {
  ...
  //   mpcTime (image creation time as a datetime) and
    ...
    success: function(data) {
           ...
           if(num) {
             ...
             $.each(rows, function(i, v) {
               ...
               image.mpcTime = v.time;       // creation time as datetime string
...
function getInfo(callback) {
  $.ajax({
    ...
    success: function(data) {
           ...
           g.allowAds = data.sites.allowAds;
           g.allowVideo = data.sites.allowVideo;
           g.playbingo = data.sites.playbingo;
           if(g.playbingo == 'yes' && g.bingotimeout === null && g.pusherChannel.subscribed === true) {
             getBingo();
           }

           g.playLotto = data.sites.playLotto;
           if(g.playLotto == 'yes') {
             var image = new Image;
