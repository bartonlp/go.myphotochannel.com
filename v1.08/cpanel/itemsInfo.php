...
include("cpanel.top.php"); // MUST add back cpanel/ when we move this to root.
...
if($_GET['debug'] && $_GET['debug'] == $S->superuser) {
  ...
  $sql = "select siteId, allowAds, allowVideo, playbingo, playLotto, perRecent, featureExt from appinfo";
  ...
  $S->query($sql);
  list($cnt) = $S->fetchrow('num');
  $sql = "select siteId, allowAds, allowVideo, playbingo, playLotto, perRecent, featureExt ".
...
FeatureExt: <b>{$v['featureExt']}</b><br>
