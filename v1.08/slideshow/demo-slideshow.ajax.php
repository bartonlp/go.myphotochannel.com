...
// Moved data from sites to playbingo. Put the data into $S->sites so I don't need to change
// everything.
...
function getItem($S) {
  ...
  // $S->category = database category. If $S->retCategory == 'frature' then this is 'photo'
...
//                    "type: $S->type, num: $S->num\n", FILE_APPEND);
  ...
  $sql = "select lifeOfFeature, whenPhotoAged from demo-appinfo where siteId='$S->siteId'";
  $S->query($sql);
  ...
  switch($S->type) {
    ...
    case 'chron':
    default:
      $order = 'order by creationTime desc';
      break;
  }
  ...
  switch($S->retCategory) {
    case 'photo':
      ...
      $per = $S->sites['perRecent'] / 100;
      
      for($j=0; $j<2; ++$j) {
        if($j == 0) {
          // First time so do recent
            
          $where = "creationTime > '$aged'"; // recent only
      ...
      $sql = "update demo-startup set lasttime=now() where id='$S->startupId'";
      $S->query($sql);
      ...
      $pusher->trigger("demo-slideshow", "startup-update", array('msg'=>"startup-update"));
...
function getItems($S, $where, $order) {
  ...
  $sql = "select * from demo-items where siteId='$S->siteId' ".
  ...
  while($row = $S->fetchrow('assoc')) {
    ...
    if(($row['type'] == 'image' || $row['type'] == 'filehtml' || $row['type'] == 'video') && !file_exists(SITE_ROOT . "/" . $row['location'])) {
      continue;
    }
...
function getInfo($S) {
  $sql = "select category, cs1, cs2, cs3, cs4, cs5 from demo-segments where siteId='$S->siteId'";
  ...
  $sql = "select lifeOfFeature, whenPhotoAged, ".
         ...
         "from demo-appinfo where siteId='$S->siteId'";
  ...
  $sql = "select category, duration, transition, effect from demo-categories where siteId='$S->siteId'";
...
//  $S->query("select allowAds, allowVideo, playbingo from sites where siteId='$S->siteId'");
//  list($allowAds, $allowVideo, $playbingo) = $S->fetchrow('num');
...
function getAds($S) {
  ...
  foreach($ads as $ad) {
    // order by creationTime (6/14/2013) //Random ordering of rows
    
    $sql = "select itemId, description, duration, location, creationTime, type, ".
           "transition, effect, skip from demo-ads ".
...
function getAdsInfo($S) {
  $S->query("select adId from demo-adsAccount order by adId");
  $result = $S->getResult();
  $adsInfo = array();
  $i = 0;
  while(list($id) = $S->fetchrow($result, 'num')) {
    $sql = "select adId, dur, trans, effect, segInfo from demo-adsInfo where adId='$id'";
    $n = $S->query($sql);
...
function getBingo($S) {
  ...
  $S->query("update demo-bingogames set gameover='yes' where siteId='$S->siteId' and unit='$S->unit'");
  ...
  $S->query("insert into demo-bingogames (siteId, unit) values('$S->siteId', '$S->unit')");
  ...
  $sql = "select itemId, location from demo-items where siteId='$S->siteId' ".
  ...
  while(list($itemId, $location) = $S->fetchrow('num')) {
    ...
    if(!file_exists(SITE_ROOT . "/" . $location)) {
      continue;
    }
  ...
  foreach($items as $item) {
    $S->query("insert into demo-bingo (itemId, siteId, game, location) ".
...
function bingoupdate($S) {
  ...
  $secret = '86714601dfa6e13a87f7';
  $pusher = new Pusher($key, $secret, $app_id);
  $pusher->trigger("demo-playbingo", "newinx", array('msg'=>$S->gameover,
                                                'inx'=>$S->inx,
                                                'game'=>$S->bingoGame));
  ...
  if($S->gameover == 'yes') {
    $sql = "update demo-bingogames set gameover='yes', inx='$S->inx' where gameNumber='$S->bingoGame'";
    $S->query($sql);
  } 
  exit();
}
...
function startup($S) {
  ...
  $sql = "insert into demo-startup (siteId, unit, version) values('$S->siteId', '$S->unit', '$S->version')";
  $S->query($sql);
  ...
  $secret = '86714601dfa6e13a87f7';
  $pusher = new Pusher($key, $secret, $app_id);
  $pusher->trigger("demo-slideshow", "startup", array('msg'=>"startup"));
...
function unload($S) {
  ...
  $secret = '86714601dfa6e13a87f7';
  $pusher = new Pusher($key, $secret, $app_id);
  $pusher->trigger("demo-slideshow", "unload", array('msg'=>"unload"));

  //file_put_contents("/tmp/debug.txt", "unload: ". date("Y-m-d H:i:s"));
  $S->query("update demo-startup set status='closed' where id='$S->startupId'");
  exit();
}
?>