...
/*
      ...
      ksort($xx);
      sort($yy);
      file_put_contents("stuff.txt", "siteId: $S->siteId, xx: ".print_r($xx, true)." yy: ".print_r($yy, true). "\n", FILE_APPEND);
*/
      // Update startup
...
function getItems($S, $where, $order) {
  ...
  while($row = $S->fetchrow('assoc')) {
    ...
    if(($row['type'] == 'image' || $row['type'] == 'filehtml' || $row['type'] == 'video') && !file_exists(DOC_ROOT . "/" . $row['location'])) {
      continue;
    }
